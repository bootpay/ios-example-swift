...
class BioController: SwipeBackController {
    let _unique_user_id = "123456abcdffffe23456789012345613245167891212"
    ...
    // 주의: secret_key (secret) 는 클라이언트에 절대 포함하지 말 것 — 서버 SDK 에서만 사용. 아래 호출은 서버에서 받은 토큰을 직접 주입하도록 변경하세요.
    let secretKey = ""
    ...
    func setUI() {
        ...
        btn.setTitle("생체인증 테스트", for: .normal)
    ...
    func bootpayStart(userToken: String) {
        ...
        BootpayBio.requestBioPayment(viewController: self, payload: payload)
                .onCancel { data in
                    print("-- cancel: \(data)")
                }
                .onIssued { data in
...
extension BioController: BootpayRestProtocol {
